...
import { getSellerTransactionsApi, getSellerBalanceApi } from "../../api/transactionApi";

const transactionTypes = ["ALL", "ORDER_SETTLEMENT", "REFUND_DEDUCTION", "PLATFORM_FEE"];
...
export default function SellerTransactions() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const { data } = await getSellerTransactionsApi(params);
  ...
  const fetchBalance = async () => {
    try {
      const { data } = await getSellerBalanceApi();
      if (data.success) {
  ...
  const getTypeBadge = (type) => {
    const typeColors = {
      ORDER_SETTLEMENT: "bg-green-500",
      REFUND_DEDUCTION: "bg-red-500",
      PLATFORM_FEE: "bg-blue-500",
    };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-gray-500">Total Platform Fees</h3>
          ...
          <div className="space-y-2">
            <h3 className="text-sm font-medium text-gray-500">Order Settlements</h3>
            <div className="flex items-center text-2xl font-bold text-green-600">
              <IndianRupee className="h-6 w-6 mr-1" />
              {balanceData?.breakdown?.orderSettlements || 0}
            </div>
          </div>

          <div className="space-y-2">
            <h3 className="text-sm font-medium text-gray-500">Refund Deductions</h3>
            <div className="flex items-center text-2xl font-bold text-red-600">
              <IndianRupee className="h-6 w-6 mr-1" />
              {balanceData?.breakdown?.refundDeductions || 0}
            </div>
          </div>
        </div>
      </Card>
