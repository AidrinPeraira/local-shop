...
export const getCartItems = asyncHandler(async (req, res) => {
  ...
  const processedItems = cart.items.map((item) => {
    ...
    const processedVariants = item.variants.map((variant) => {
      ...
      const basePrice = productVariant
      ...
      if (applicableDiscount) {
        variantDiscount =
          (basePrice * applicableDiscount.priceDiscountPerUnit) / 100;
      }
  ...
  const cartTotal =
    subtotalBeforeDiscount - totalDiscount + shippingCharge + platformFee;
...
export const updateCart = asyncHandler(async (req, res) => {
  ...
  for (const variant of variants) {
    ...
    if (
      productVariant &&
      (!productVariant.inStock || productVariant.stock < variant.quantity)
    ) {
      res.status(HTTP_CODES.BAD_REQUEST);
      throw new Error(
        `Variant in product ${product.productName} is out of stock or has insufficient quantity`
      );
    }
  }
  ...
  for (const item of cart.items) {
    ...
    const updatedVariants = variants.filter(
      (v) => v.productId.toString() === item.product.toString()
    );
...
export const processCartItems = asyncHandler(async (req, res) => {
  const userId = req.user._id;
  const { productId, variants } = req.body;
