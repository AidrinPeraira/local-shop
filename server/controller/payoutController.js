...
export const processVendorPayouts = asyncHandler(async (req, res) => {
  const { payoutIds } = req.body;

  try {
    const updatedPayouts = await Transaction.updateMany(
      {
        _id: { $in: payoutIds },
        status: { $ne: "COMPLETED" }
      },
    ...
    res.json({
      ...
      modifiedCount: updatedPayouts.modifiedCount
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
export const processBuyerRefunds = asyncHandler(async (req, res) => {
  const { refundIds } = req.body;

  try {
    const updatedRefunds = await Transaction.updateMany(
      {
        _id: { $in: refundIds },
        status: { $ne: "COMPLETED" }
      },
    ...
    res.json({
      ...
      modifiedCount: updatedRefunds.modifiedCount
    });
  } catch (error) {
    res.status(500).json({
      success: false,
